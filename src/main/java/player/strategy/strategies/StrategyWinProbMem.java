...
public class StrategyWinProbMem implements IStrategy, IMemory {
  ...
  public Card recommendedChoose(List<Card> hand, List<Card> pond, List<Card> well) {
    ...
    List<Integer> idxsOfHighestPerms = new ArrayList<>();
    int largest = 0;
    for (int i = 0; i < pursueByCCPerms.size(); i++) {
      int perms = pursueByCCPerms.get(i);
      if (perms > largest) {
        largest = perms;
        idxsOfHighestPerms = new ArrayList<>();
  ...
  public void notifyOfPlayerDiscard(IPlayer p, Card c, List<Card> well) {
    if (new Random().nextInt(0, 101) <= accuracy) discarded.remove(c);
  }
  ...
  public void notifyOfPlayerChoice(IPlayer p, Card c, String location, List<Card> well,
                                   List<Card> pond) {
    if (new Random().nextInt(0, 101) <= accuracy) discarded.add(c);
  }

  // TODO: javadoc
  @Override
  public void notifyOfCardsCleared(List<Card> cards) {
    if (new Random().nextInt(0, 101) <= accuracy) discarded.addAll(cards);
  }
  ...
  private void getPursuableWHs(List<Card> hand) {
    ...
    if (WinningHand.cardsAwayFromClosestTo(hand) > 2 || closestTo.isEmpty()) return;
  ...
  private List<Integer> getPursuableByCCCumulativePerms(List<List<WinningHand>> WHLists,
    ...
    for (List<WinningHand> l : WHLists) {
      ...
      for (WinningHand w : l) {
        cumulativePerms += WinningHand.permsWOHasAndDiscarded(w, hand, discarded);
      }
  ...
  private List<WinningHand> notPursuableWithBlues(List<WinningHand> WHs, List<Card> hand) {
    ...
    for (WinningHand w : WHs) {
      ...
      switch (bluesNeeded.size()) {
        ...
        // if four blue cards, add w to notPursuable if hand doesn't contain 3+ of those cards or if
        //   any of those cards are in discarded
        case 4 -> {
          List<UCard> neededBlues = new ArrayList<>();
          for (UCard c : UCard.toUCards(hand)) {
            if (bluesNeeded.contains(c)) neededBlues.add(c);
          }
          if (neededBlues.size() > 1
                  || !discarded.contains(new Card(neededBlues.getFirst().cards().getFirst())))
            notPursuable.add(w);
        }
        // if five blue cards, add w to notPursuable if hand doesn't contain 4+ of those cards or if
        //   any of those cards are in discarded
        case 5 -> {
          List<UCard> neededBlues = new ArrayList<>();
          for (UCard c : UCard.toUCards(hand)) {
  ...
  private void sortIdealChoices(List<Card> hand) {
    ...
    List<Card> ICCopy = new ArrayList<>(idealChoices);
